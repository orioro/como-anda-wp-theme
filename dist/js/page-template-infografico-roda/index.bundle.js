var pagetemplateinfograficoroda = (function () {
  ...
  var arcTextTransform = function (options) {
    ...
    return function (d) {
      var midAngle = (d.startAngle + d.endAngle) / 2;
      return 'rotate(' + (midAngle * 180 / Math.PI - 90) + ')' + 'translate(' + options.radius + ')' + (midAngle > Math.PI && midAngle <= 2 * Math.PI ? 'rotate(180)' : '');
    };
  };
    ...
    */
    ...
    d3.json(app.config.states_geo_json_url, function (err, geoData) {
      ...
      mapFilter.on('change', function () {
        ...
        mapContainer.selectAll('path').attr('class', function (d) {
      ...
      }).attr('stroke', 'black'); // once the map has been rendered, put it into the right
      // place
  ...
  var stateDropdown = function (app, options) {
    var $stateDropdown = jQuery('#state-dropdown');
    var $stateCheckboxes = $stateDropdown.find('input[type="checkbox"][name="state"]');
    var $allCheckbox = $stateDropdown.find('input[type="checkbox"][name="_all"]');
    var ALL_STATE_CODES = Array.from($stateCheckboxes).map(function (checkbox) {
      return checkbox.getAttribute('value');
    });
    $(window).on('click', function (_ref) {
    ...
    $stateDropdown.on('click', 'button', function (e) {
      $stateDropdown.toggleClass('active');
    });
    $stateCheckboxes.on('change', function () {
      var selectedStates = Array.from($stateDropdown.find('input[type="checkbox"]:checked')).map(function (checkbox) {
        return checkbox.getAttribute('value');
      });
  ...
  var entities = function (app, options) {
    ...
    function update(entities) {
      ...
      stateArcs.select('text').style('text-anchor', function (d) {
        ...
        var midAngle = (d.startAngle + d.endAngle) / 2 - d.padAngle;
        return 'rotate(' + (midAngle * 180 / Math.PI - 90) + ')' + 'translate(' + (options.innerRadius + 26) + ')' + (midAngle > Math.PI && midAngle <= 2 * Math.PI ? 'rotate(180)' : '');
      });
  ...
  var init$3 = function (app, options) {
    ...
    */
    ...
    app.ui.stateDropdown = stateDropdown(app, {}); // listen to map filter changes
  ...
  window.addEventListener('DOMContentLoaded', function () {
    ...
    d3.json(app.config.typeform_data_url, function (err, TF_DATA) {
      ...
      init$3(app, options); // open criteria
